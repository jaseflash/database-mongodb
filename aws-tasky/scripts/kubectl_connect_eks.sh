...
output=$(terraform output -state $SCRIPT_DIR/../quickstart/infra/terraform.tfstate -json)
else
...
echo
echo "Create Aliases"
echo "  alias: ${EKS_CLUSTER_NAME} =  kubectl config use-context ${CLUSTER_NAME} # ${EKS_CLUSTER_NAMES##*-}"
# alias $(echo ${EKS_CLUSTER_NAME})="kubectl config use-context ${CLUSTER_NAME} # ${EKS_CLUSTER_NAMES##*-}"
